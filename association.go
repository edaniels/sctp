...
const (
	...
	// TODO(erd): making this 128 makes the issue go away
...
type Association struct {
	...
	packetsWithDataSinceLastSACK uint8
	// per inbound packet context
	immediateSackTriggered bool

	name string
	log  leveledLoggerWithName

	// TODO(erd): remove these after testing or incorporate them to trace
	lastSACK    time.Time
	lastPkt     time.Time
	lastPktSize int
}
...
func createClientWithContext(ctx context.Context, config Config) (*Association, error) {
	...
	select {
	case <-ctx.Done():
		a.log.Errorf("client handshake canceled: state=%s", getAssociationStateString(a.getState()))
...
func createAssociation(config Config) *Association {
	...
	go func() {
		...
		for {
			...
			fmt.Printf("[%s] stats inflight queue bytes: %d\n", a.name, a.inflightQueue.getNumBytes())
			fmt.Printf("[%s] stats pending queue size: %d\n", a.name, a.pendingQueue.size())
			fmt.Printf("[%s] stats CWND: %d\n", a.name, a.CWND())
			fmt.Printf("[%s] stats peer's rwnd: %d\n", a.name, a.rwnd)
			fmt.Printf("[%s] stats my rwnd: %d\n", a.name, a.getMyReceiverWindowCredit())
			fmt.Printf("[%s] stats nSACKs (in) : %d\n", a.name, a.stats.getNumSACKs())
			fmt.Printf("[%s] stats nSACKs (out) : %d\n", a.name, a.stats.getNumSACKsSent())
			fmt.Printf("[%s] stats nT3Timeouts : %d\n", a.name, a.stats.getNumT3Timeouts())
			fmt.Printf("[%s] stats nAckTimeouts: %d\n", a.name, a.stats.getNumAckTimeouts())
			fmt.Printf("[%s] stats nFastRetrans: %d\n", a.name, a.stats.getNumFastRetrans())
			bytesQueued := []uint32{}
			readable := []bool{}
			for _, s := range a.streams {
				bytesQueued = append(bytesQueued, uint32(s.getNumBytesInReassemblyQueue()))
				readable = append(readable, s.reassemblyQueue.isReadable())
			}
			fmt.Printf("[%s] stats reassm byte size: %v %v\n", a.name, bytesQueued, readable)
		}
	}()

	a.log = newLeveledLoggerWithName(config.LoggerFactory.NewLogger("sctp"), a.name)

	// RFC 4690 Sec 7.2.1
	//  o  The initial cwnd before DATA transmission or after a sufficiently
	//     long idle period MUST be set to min(4*MTU, max (2*MTU, 4404
	//     bytes)).
	a.setCWND(min32(4*a.MTU(), max32(2*a.MTU(), 4404)))
	// fmt.Printf(a.name+" updated5 usable cwnd=%d ssthresh=%d inflight=%d (INI)",
...
func (a *Association) init(isClient bool) {
	...
	if isClient {
		...
		err := a.sendInit()
		if err != nil {
			a.log.Errorf("failed to send init: %s", err.Error())
		}
...
func (a *Association) Shutdown(ctx context.Context) error {
	a.log.Debug("closing association..")
...
func (a *Association) Close() error {
	...
	a.log.Debug("association closed")
	a.log.Debugf("stats nDATAs (in) : %d", a.stats.getNumDATAs())
	a.log.Debugf("stats nSACKs (in) : %d", a.stats.getNumSACKs())
	a.log.Debugf("stats nT3Timeouts : %d", a.stats.getNumT3Timeouts())
	a.log.Debugf("stats nAckTimeouts: %d", a.stats.getNumAckTimeouts())
	a.log.Debugf("stats nFastRetrans: %d", a.stats.getNumFastRetrans())
...
func (a *Association) Abort(reason string) {
	a.log.Debugf("aborting association: %s", reason)
...
func (a *Association) readLoop() {
	...
	a.log.Debugf("readLoop exited %s", closeErr)
}

func (a *Association) writeLoop() {
	a.log.Debug("writeLoop entered")
	defer a.log.Debug("writeLoop exited")
	...
	for {
		...
		for _, pkt := range rawPackets {
			...
			for _, chunk := range p.chunks {
				...
				case *chunkPayloadData:
					_ = c
					chunkDiag.sendChunk(c)
					// fmt.Println(a.name, "wrote ssn", c.streamSequenceNumber, c.tsn)
				}
			}
		}

		// fmt.Println("will write", len(rawPackets))
		for _, raw := range rawPackets {
			n, err := a.netConn.Write(raw)
			if err != nil {
				if !errors.Is(err, io.EOF) {
					panic("HERE3")
					a.log.Warnf("failed to write packets on netConn: %v", err)
				}
				panic("HERE4")
				a.log.Debug("writeLoop ended")
				break loop
			}
			if n != len(raw) {
				panic("what...")
		...
		if !ok {
			if err := a.close(); err != nil {
				a.log.Warnf("failed to close association: %v", err)
			}

			return
		}

		// fmt.Println(a.name, "sent", len(rawPackets), "now going to sleep with queue", a.pendingQueue.size(), a.inflightQueue.getNumBytes(), a.inflightQueue.size())
		// now := time.Now()
		select {
		case <-a.awakeWriteLoopCh:
			// fmt.Println(a.name, "slept for...", time.Since(now))
		case <-a.closeWriteLoopCh:
			break loop
		}
	}
...
func (a *Association) handleInbound(raw []byte) error {
	now := time.Now()
	// fmt.Printf("[%s]: last packet received: %s size: %d->%d\n", a.name, now.Sub(a.lastPkt), a.lastPktSize, len(raw))
	a.lastPkt = now
	a.lastPktSize = len(raw)
	p, err := a.unmarshalPacket(raw)
	if err != nil {
		a.log.Warnf("unable to parse SCTP packet %s", err)
		panic("HERE1")
	...
	numDATAsAtStart := a.handleChunksStart()
	...
	a.handleChunksEnd(numDATAsAtStart)
...
func (a *Association) gatherOutboundDataAndReconfigPackets(rawPackets [][]byte) [][]byte {
	...
	if len(chunks) > 0 {
		...
		a.log.Trace("T3-rtx timer start (pt1)")
	...
	if len(sisToReset) > 0 || a.willRetransmitReconfig {
		if a.willRetransmitReconfig {
			...
			a.log.Debugf("retransmit %d RECONFIG chunk(s)", len(a.reconfigs))
			for _, c := range a.reconfigs {
				p := a.createPacket([]chunk{c})
				raw, err := a.marshalPacket(p)
				if err != nil {
					a.log.Warn("failed to serialize a RECONFIG packet to be retransmitted")
				} else {
					rawPackets = append(rawPackets, raw)
				}
			}
		...
		if len(sisToReset) > 0 {
			...
			a.log.Debugf("sending RECONFIG: rsn=%d tsn=%d streams=%v",
				rsn, a.myNextTSN-1, sisToReset)
			p := a.createPacket([]chunk{c})
			raw, err := a.marshalPacket(p)
			if err != nil {
				a.log.Warn("failed to serialize a RECONFIG packet to be transmitted")
			} else {
				rawPackets = append(rawPackets, raw)
			}
		}
...
func (a *Association) gatherOutboundFastRetransmissionPackets(rawPackets [][]byte) [][]byte {
	if a.willRetransmitFast {
		...
		for i := 0; ; i++ {
			...
			a.log.Tracef("fast-retransmit: tsn=%d sent=%d htna=%d",
				c.tsn, c.nSent, a.fastRecoverExitPoint)
		}
...
func (a *Association) gatherOutboundSackPackets(rawPackets [][]byte) [][]byte {
	if a.ackState == ackStateImmediate {
		...
		now := time.Now()
		a.log.Debugf("sending SACK\n: %s", sack)
		a.stats.incSACKsSent()
		a.log.Tracef("last SACK sent: %s", now.Sub(a.lastSACK))
		// fmt.Printf("[%s]: last SACK sent: %s -> %d:%d:%d\n", a.name, now.Sub(a.lastSACK), a.stats.getNumPackets(), a.stats.getNumDATAs(), a.stats.getNumSACKsSent())
		a.lastSACK = now
...
func (a *Association) gatherOutboundShutdownPackets(rawPackets [][]byte) ([][]byte, bool) {
	...
	case a.willSendShutdown:
		...
		if err != nil {
			a.log.Warn("failed to serialize a Shutdown packet")
		} else {
	...
	case a.willSendShutdownAck:
		...
		if err != nil {
			a.log.Warn("failed to serialize a ShutdownAck packet")
		} else {
			a.t2Shutdown.start(a.rtoMgr.getRTO())
			rawPackets = append(rawPackets, raw)
		}
	case a.willSendShutdownComplete:
		...
		if err != nil {
			a.log.Warn("failed to serialize a ShutdownComplete packet")
		} else {
			rawPackets = append(rawPackets, raw)
			ok = false
		}
...
func (a *Association) gatherOutbound() ([][]byte, bool) {
	...
	if a.willSendAbort {
		...
		if err != nil {
			a.log.Warn("failed to serialize an abort packet")
			return nil, false
		}
...
func (a *Association) handleInit(p *packet, i *chunkInit) ([]*packet, error) {
	state := a.getState()
	a.log.Debugf("chunkInit received in state '%s'", getAssociationStateString(state))
	...
	for _, param := range i.params {
		...
		case *paramSupportedExtensions:
			for _, t := range v.ChunkTypes {
				if t == ctForwardTSN {
					a.log.Debug("use ForwardTSN (on init)")
	...
	if !a.useForwardTSN {
		a.log.Warn("not using ForwardTSN (on init)")
	}
	...
	a.log.Debugf("useZeroChecksum=%t (on init)", a.getUseZeroChecksum())
...
func (a *Association) handleInitAck(p *packet, i *chunkInitAck) error {
	state := a.getState()
	a.log.Debugf("chunkInitAck received in state '%s'", getAssociationStateString(state))
	if state != cookieWait {
		// RFC 4960
	...
	//     advertised window).
	a.ssthresh = a.RWND()
	// fmt.Printf(a.name+" updated6 usable cwnd=%d ssthresh=%d inflight=%d (INI)",
	// a.CWND(), a.ssthresh, a.inflightQueue.getNumBytes())
	...
	for _, param := range i.params {
		...
		case *paramSupportedExtensions:
			for _, t := range v.ChunkTypes {
				if t == ctForwardTSN {
					a.log.Debug("use ForwardTSN (on initAck)")
					a.useForwardTSN = true
				}
			}
		case *paramZeroChecksumAcceptable:
	...
	a.log.Debugf("useZeroChecksum=%t (on initAck)", a.getUseZeroChecksum())

	if !a.useForwardTSN {
		a.log.Warn("not using ForwardTSN (on initAck)")
	}
	if cookieParam == nil {
...
func (a *Association) handleCookieEcho(c *chunkCookieEcho) []*packet {
	state := a.getState()
	a.log.Debugf("COOKIE-ECHO received in state '%s'", getAssociationStateString(state))

	if a.myCookie == nil {
		a.log.Debug("COOKIE-ECHO received before initialization")
		return nil
	}
	switch state {
	default:
...
func (a *Association) handleCookieAck() {
	state := a.getState()
	a.log.Debugf("COOKIE-ACK received in state '%s'", getAssociationStateString(state))
...
var couldPush map[int]bool = map[int]bool{}
var couldPushMu sync.Mutex

// The caller should hold the lock.
func (a *Association) handleData(d *chunkPayloadData) []*packet {
	rtxMu.Lock()
	if rtx[d.tsn] {
		// println("got a rtx!", d.tsn, d.streamSequenceNumber)
	} else {
		// println(time.Now().Unix(), a.name, "got ssn", d.streamSequenceNumber, d.tsn)
	}
	rtxMu.Unlock()
	chunkDiag.receiveChunk(d)
	// a.log.Errorf("DATA: tsn=%d immediateSack=%v len=%d",
	// 	d.tsn, d.immediateSack, len(d.userData))
	a.stats.incDATAs()

	canPush := a.payloadQueue.canPush(d, a.peerLastTSN)
	if canPush {
		couldPushMu.Lock()
		couldPush[int(d.streamSequenceNumber)] = true
		couldPushMu.Unlock()
		// https://www.rfc-editor.org/rfc/rfc9260#section-6.2-3 we have 200 ms
		// to respond unless we receive enough DATA before then.
		a.ackTimer.start()
		...
		defer func() {
			rtxMu.Lock()
			if rtx[d.tsn] {
				// println("got a rtx2!", d.tsn, d.streamSequenceNumber, s.reassemblyQueue.isReadable())
			}
			rtxMu.Unlock()
		}()
		...
		if a.getMyReceiverWindowCredit() > 0 {
			...
			if ok && sna32LT(d.tsn, lastTSN) {
				fmt.Printf("receive buffer full, but accepted as this is a missing chunk with tsn=%d ssn=%d", d.tsn, d.streamSequenceNumber)
				a.payloadQueue.push(d, a.peerLastTSN)
				s.handleData(d)
			} else {
				fmt.Printf("receive buffer full. dropping DATA with tsn=%d ssn=%d", d.tsn, d.streamSequenceNumber)
			}
		}
	} else {
		couldPush[int(d.streamSequenceNumber)] = false
	}
...
func (a *Association) handlePeerLastTSNAndAcknowledgement(sackImmediately bool) []*packet {
	...
	if hasPacketLoss {
		a.log.Tracef("packetloss: %s", a.payloadQueue.getGapAckBlocksString(a.peerLastTSN))
	}

	if sackImmediately {
		a.immediateSackTriggered = true
	}
...
func (a *Association) getMyReceiverWindowCredit() uint32 {
	...
	for _, s := range a.streams {
		...
		// fmt.Println(a.name, "bytes in my reassem queue", bytesQueued, s.reassemblyQueue.isReadable())
	}
...
func (a *Association) createStream(streamIdentifier uint16, accept bool) *Stream {
	s := &Stream{
		...
		log:              a.log.withName(fmt.Sprintf("%d", streamIdentifier)),
	}
	...
	if accept {
		...
		case a.acceptCh <- s:
			...
			a.log.Debugf("accepted a new stream (streamIdentifier: %d)",
				streamIdentifier)
		default:
			a.log.Debugf("dropped a new stream (acceptCh size: %d)",
				len(a.acceptCh))
			return nil
		}
	} else {
	...
	for i := a.cumulativeTSNAckPoint + 1; sna32LTE(i, d.cumulativeTSNAck); i++ {
		...
		if !c.acked {
			...
			if c.nSent == 1 && sna32GTE(c.tsn, a.minTSN2MeasureRTT) {
				...
				a.srtt.Store(srtt)
				a.log.Tracef("SACK: measured-rtt=%f srtt=%f new-rto=%f",
					rtt, srtt, a.rtoMgr.getRTO())
			}
		}
...
func (a *Association) onCumulativeTSNAckPointAdvanced(totalBytesAcked int) {
	...
	if a.inflightQueue.size() == 0 {
		...
		a.log.Trace("T3-rtx timer start (pt2)")
		a.t3RTX.start(a.rtoMgr.getRTO())
	}
	...
	if a.CWND() <= a.ssthresh {
		...
		if !a.inFastRecovery &&
			...
			// TODO(erd): CWND vs MTU here could make a difference?
			// See https://datatracker.ietf.org/doc/html/rfc3465#section-2.3 for choice of 2*a.MTU
			// current := a.CWND()
			new := a.CWND() + min32(uint32(totalBytesAcked), 2*a.MTU())
			a.setCWND(new)
			// a.cwnd += min32(uint32(totalBytesAcked), a.MTU()) // SCTP way (slow)
			// fmt.Printf(a.name+" updated1 usable cwnd=%d last=%d factor=%f ssthresh=%d acked=%d (SS)",
			// current, new, float64(new)/float64(current), a.ssthresh, totalBytesAcked)
		} else {
			a.log.Tracef("cwnd did not grow: cwnd=%d ssthresh=%d acked=%d FR=%v pending=%d",
				a.CWND(), a.ssthresh, totalBytesAcked, a.inFastRecovery, a.pendingQueue.size())
		}
	} else {
		...
		if a.partialBytesAcked >= a.CWND() && a.pendingQueue.size() > 0 {
			...
			// fmt.Printf(a.name+" updated2 usable cwnd=%d ssthresh=%d acked=%d (CA)",
			// a.CWND(), a.ssthresh, totalBytesAcked)
		}
	}
}
			...
			if !c.acked && !c.abandoned() && c.missIndicator < 3 {
				...
				if c.missIndicator == 3 {
					if !a.inFastRecovery {
						...
						// fmt.Printf(a.name+" updated3 usable cwnd=%d ssthresh=%d inflight=%d (FR)",
						// a.CWND(), a.ssthresh, a.inflightQueue.getNumBytes())
					}
				}
			}
...
func (a *Association) handleSack(d *chunkSelectiveAck) error {
	...
	if bytesOutstanding >= d.advertisedReceiverWindowCredit {
		fmt.Println(a.name, "set peer rwnd1", 0, bytesOutstanding, d.advertisedReceiverWindowCredit)
		a.setRWND(0)
	} else {
		a.setRWND(d.advertisedReceiverWindowCredit - bytesOutstanding)
		// fmt.Println(a.name, "set peer rwnd2", d.advertisedReceiverWindowCredit, "-", bytesOutstanding, "=", d.advertisedReceiverWindowCredit-bytesOutstanding)
	}
...
func (a *Association) postprocessSack(state uint32, shouldAwakeWriteLoop bool) {
	...
	case a.inflightQueue.size() > 0:
		// Start timer. (noop if already started)
		a.log.Trace("T3-rtx timer start (pt3)")
		a.t3RTX.start(a.rtoMgr.getRTO())
...
func (a *Association) handleAbort(c *chunkAbort) error {
	...
	return fmt.Errorf("%w: %s", ErrChunk, errStr)
}
...
func (a *Association) createForwardTSN() *chunkForwardTSN {
	...
	a.log.Tracef("building fwdtsn: newCumulativeTSN=%d cumTSN=%d - %s", fwdtsn.newCumulativeTSN, a.cumulativeTSNAckPoint, streamStr)
...
func (a *Association) handleForwardTSN(c *chunkForwardTSN) []*packet {
	...
	if !a.useForwardTSN {
		a.log.Warn("received FwdTSN but not enabled")
	...
	a.log.Tracef("should send ack? newCumTSN=%d peerLastTSN=%d",
		c.newCumulativeTSN, a.peerLastTSN)
	if sna32LTE(c.newCumulativeTSN, a.peerLastTSN) {
		a.log.Trace("sending ack on Forward TSN")
...
func (a *Association) handleReconfigParam(raw param) (*packet, error) {
	...
	case *paramOutgoingResetRequest:
		a.log.Trace("handleReconfigParam (OutgoingResetRequest)")
	...
	case *paramReconfigResponse:
		a.log.Trace("handleReconfigParam (ReconfigResponse)")
...
func (a *Association) resetStreamsIfAny(p *paramOutgoingResetRequest) *packet {
	...
	if sna32LTE(p.senderLastTSN, a.peerLastTSN) {
		a.log.Debugf("resetStream(): senderLastTSN=%d <= peerLastTSN=%d",
			p.senderLastTSN, a.peerLastTSN)
		for _, id := range p.streamIdentifiers {
			...
			a.lock.Lock()
			a.log.Debugf("deleting stream %d", id)
			delete(a.streams, s.streamIdentifier)
		}
...
func (a *Association) movePendingDataChunkToInflightQueue(c *chunkPayloadData) {
	...
	a.log.Tracef("sending ppi=%d tsn=%d ssn=%d sent=%d len=%d (%v,%v)",
		c.payloadType, c.tsn, c.streamSequenceNumber, c.nSent, len(c.userData), c.beginningFragment, c.endingFragment)
	...
	if s, ok := a.streams[c.streamIdentifier]; ok {
		...
		if s.reliabilityType == ReliabilityTypeRexmit {
			if c.nSent >= s.reliabilityValue {
				c.setAbandoned(true)
				a.log.Tracef("marked as abandoned: tsn=%d ppi=%d (remix: %d)", c.tsn, c.payloadType, c.nSent)
			}
			...
			if elapsed >= int64(s.reliabilityValue) {
				c.setAbandoned(true)
				a.log.Tracef("marked as abandoned: tsn=%d ppi=%d (timed: %d)", c.tsn, c.payloadType, elapsed)
			}
		}
		s.lock.RUnlock()
	} else {
		a.log.Errorf("stream %d not found)", c.streamIdentifier)
	}
}
...
func (a *Association) getDataPacketsToRetransmit() []*packet {
	...
	for i := 0; !done; i++ {
		...
		// fmt.Println(a.netConn.LocalAddr().String(), "retransmitting", c.streamSequenceNumber, c.tsn, c.nSent)
		rtxMu.Lock()
		rtx[c.tsn] = true
		rtxMu.Unlock()
	...
	pkts := a.bundleDataChunksIntoPackets(chunks)
	if len(pkts) > 0 {
		fmt.Println("rtxmit", len(pkts))
	}
	return pkts
}

var rtx map[uint32]bool = map[uint32]bool{}
var rtxMu sync.Mutex
...
func (a *Association) handleChunksStart() (numDATAs uint64) {
	a.lock.Lock()
	defer a.lock.Unlock()

	a.stats.incPackets()

	a.immediateSackTriggered = false
	return a.stats.getNumDATAs()
}

func (a *Association) handleChunksEnd(numDATAsAtStart uint64) {
	a.lock.Lock()
	defer a.lock.Unlock()

	// TODO(erd): this does not appear to fix https://github.com/pion/sctp/issues/181
	if a.stats.getNumDATAs() > numDATAsAtStart {
		a.packetsWithDataSinceLastSACK++
	}
	if a.packetsWithDataSinceLastSACK >= 20 || a.immediateSackTriggered {
		a.packetsWithDataSinceLastSACK = 0
		a.ackState = ackStateImmediate
		a.ackTimer.stop()
		a.awakeWriteLoop()
		return
...
func (a *Association) handleChunk(p *packet, c chunk) error {
	...
	if _, err = c.check(); err != nil {
		a.log.Errorf("[ %s ] failed validating chunk: %s ", err)
		panic("meow")
		return nil
	}
	...
	// TODO(erd): chunkHeartbeatAck not handled?
	case *chunkHeartbeat:
...
func (a *Association) onRetransmissionTimeout(id int, nRtos uint) {
	...
	if id == timerT2Shutdown {
		a.log.Debugf("retransmission of shutdown timeout (nRtos=%d): %v", nRtos)
		state := a.getState()
	...
	if id == timerT3RTX {
		...
		a.ssthresh = max32(a.CWND()/2, 4*a.MTU())
		a.setCWND(a.MTU())
		// fmt.Printf(a.name+" updated4 usable cwnd=%d ssthresh=%d inflight=%d (RTO)",
		// a.CWND(), a.ssthresh, a.inflightQueue.getNumBytes())
		...
		fmt.Printf("T3-rtx timed out: nRtos=%d cwnd=%d ssthresh=%d", nRtos, a.CWND(), a.ssthresh)
...
func (a *Association) onRetransmissionFailure(id int) {
	...
	if id == timerT2Shutdown {
		a.log.Error("retransmission failure: T2-shutdown")
		return
	}
...
func (a *Association) onAckTimeout() {
	...
	a.log.Tracef("ack timed out (ackState: %d)", a.ackState)
	a.stats.incAckTimeouts()

	a.ackState = ackStateImmediate
	// println(a.name, "TRIG.TIMEOUT")
	a.awakeWriteLoop()
}
